...
use rp_protocol::{
    ApprovalContext, CoseSign1, DecisionSubmission, Domain, KeyRole, Request, VerificationKey,
    VerificationPolicy, decode,
};
...
fn positive_vectors_are_stable_and_match_digests() {
    ...
    for vector in manifest.vectors {
        ...
        match vector.schema.as_str() {
            ...
            "ApprovalContext" => {
                let value = ApprovalContext::decode(&bytes)
                    .unwrap_or_else(|error| panic!("{}: {error}", vector.name));
                assert_eq!(value.canonical_bytes().unwrap(), bytes, "{}", vector.name);
                assert_eq!(
                    hex::encode(value.digest().unwrap()),
                ...
                assert_eq!(
                    hex::encode(value.webauthn_challenge().unwrap()),
                    vector.webauthn_challenge_hex.unwrap(),
                    "{}",
                    vector.name
                );
            }
            "DecisionSubmission" => {
                let value = DecisionSubmission::decode(&bytes)
                    .unwrap_or_else(|error| panic!("{}: {error}", vector.name));
                assert_eq!(value.canonical_bytes().unwrap(), bytes, "{}", vector.name);
...
fn cose_known_answer_vectors_verify_with_exact_external_aad() {
    let manifest: CoseManifest =
        serde_json::from_slice(&fs::read(vectors_path("cose-v1/manifest.json")).unwrap()).unwrap();
    assert_eq!(manifest.version, 1);
    for vector in manifest.vectors {
        let envelope = CoseSign1::decode(&hex::decode(&vector.cose_hex).unwrap())
            .unwrap_or_else(|error| panic!("{}: {error}", vector.name));
        assert_eq!(
        ...
        let key = VerificationKey::new(
            ...
            0,
            100,
            vec![match vector.role.as_str() {
                "broker" => KeyRole::Broker,
                role => panic!("{}: unsupported role {role}", vector.name),
            }],
        )
        .unwrap();
        let domain = match vector.domain.as_str() {
            "request" => Domain::Request,
            domain => panic!("{}: unsupported domain {domain}", vector.name),
        };
        envelope
            .verify(
                &key,
...
fn negative_vectors_fail_closed() {
    let manifest: NegativeManifest =
        serde_json::from_slice(&fs::read(vectors_path("negative/manifest.json")).unwrap()).unwrap();
    assert_eq!(manifest.version, 1);
    for vector in manifest.vectors {
        let bytes = hex::decode(&vector.cbor_hex).unwrap();
        let rejected = match vector.target.as_str() {
            ...
            target => panic!(
                "{}: unsupported negative vector target {target}",
                vector.name
            ),
        };
        assert!(
