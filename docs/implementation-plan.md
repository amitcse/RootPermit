...
| M2 broker | Typed package intake, durable SQLite migrations and checked file opening, canonical signed request/receipt records, pinned-WebAuthn decision CAS, recovery holds, Linux `SOCK_SEQPACKET` peer identity, and fake-plan lifecycle coverage. | Linux CI process harness evidence (including two real UIDs) and the remaining root-admin dispatcher/CLI wiring; see [`m2-local-broker.md`](evidence/m2-local-broker.md). |
